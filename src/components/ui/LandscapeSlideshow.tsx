...
interface LandscapeSlideshowProps {
  images: string[]
  className?: string
}

const LandscapeSlideshow: React.FC<LandscapeSlideshowProps> = ({ images, className = "" }) => {
  ...
  const handleScroll = (direction: "left" | "right") => {
    ...
    const cardWidth = 400 // Fixed card width for landscape
    ...
    <div className={`relative ${className}`}>
      ...
      >
        {images.map((image, index) => (
          <motion.div
            key={index}
            className="flex-shrink-0 w-[400px] md:w-[500px] snap-start"
...
export default LandscapeSlideshow
